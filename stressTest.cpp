...
public:	
	...
	void render() {
		...
		int stu = 0,mov = 0; Matrix4d mat4 = Matrix4d::Identity();
...
void idle_func() {
	...
	while (step--) {
		...
		double vis = 0.125*idle_num; int vist = vis/10; vis -= vist*10.0; if (vis > 5) vis = 5-(vis-5); vis/=2.0;
		// vis = 0.0;
		...
		Matrix3d Rotate;
		Rotate = AngleAxisd(vis*5*M_PI/180, -1*Vector3d::UnitX());
